resource "aws_instance" "rpsweb" {
  ...
  tags = {
    Name = "ubuntu-websever-moni-2"   # change the name
  }

  provisioner "file" {
    source      = "C:\\Users\\Administrator\\jpmc-main\\ubuntu-webserver\\apache2.sh"
  ...
  connection {
    ...
    user        = "ubuntu"
    # insecure    = "true"
    private_key = file("C:\\Users\\Administrator\\jpmc-main\\ubuntu-webserver\\web-server.pem")
    host        = self.public_ip
  }
}
